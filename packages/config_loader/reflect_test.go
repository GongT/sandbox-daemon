...
func (ctx *testCtx) HasValue(tagPath paths.ConfigPath) (bool, error) {
	return true, nil
}

func (ctx *testCtx) GetArraySize(tagPath paths.ConfigPath) (int, error) {
	require.Equal(ctx.t, tagPath.Size, 1)
	require.Contains(ctx.t, []string{"strings", "objects"}, tagPath.StringAt(0))
	return 2, nil
}

func (ctx *testCtx) GetObjectKeys(tagPath paths.ConfigPath) ([]string, error) {
	require.Equal(ctx.t, tagPath.Size, 1)
	require.Equal(ctx.t, "map", tagPath.StringAt(0))
	return []string{"keyA", "keyB"}, nil
}

func (ctx *testCtx) GetValue(typ reflect.Type, tagPath paths.ConfigPath) (any, error) {
